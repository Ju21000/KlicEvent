...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const fetchUserDataAndEvents = async () => {
      // On teste d'abord getSession() qui lit le stockage local du navigateur de manière synchrone/rapide
    ...
    <main className="min-h-screen bg-slate-950 text-white p-6 md:p-12">
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="bg-slate-900 border border-slate-800 p-4 rounded-xl flex justify-between items-center text-xs">
          ...
          <button onClick={handleLogout} className="text-red-400 hover:underline">Se déconnecter</button>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-12 bg-red-950/20 border border-red-900/50 rounded-2xl space-y-4">
            ...
            <Link href="/" className="inline-block px-6 py-2 bg-slate-800 rounded-xl text-sm">Retourner à l'accueil pour te connecter</Link>
          </div>
        ) : events.length === 0 ? (
          <div className="text-center py-12 bg-slate-900/50 rounded-2xl border border-slate-800 space-y-4">
            ...
            <Link href="/create-event" className="inline-block px-6 py-3 bg-purple-600 rounded-xl font-semibold">Créer mon premier événement 🚀</Link>
          </div>
        ) : (
