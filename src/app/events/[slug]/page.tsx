...
export default function EventPage({ params }: { params: Promise<{ slug: string }> }) {
        ...
        .on(
          ...
          (payload: { new: any }) => {
            setPhotos((prevPhotos) => {
              const updated = [payload.new, ...prevPhotos];
              return updated;
            });
          }
        )
  ...
  // Défilement automatique du diaporama plein écran toutes les 4 secondes
  useEffect(() => {
    ...
    <main className="min-h-screen bg-slate-950 text-white p-6 md:p-12">
      <div className="max-w-6xl mx-auto space-y-8">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-xl gap-4">
          ...
          <div className="flex items-center gap-3 w-full md:w-auto flex-wrap">
            {/* Bouton pour lancer le mode Diaporama Plein Écran */}
            <button
              onClick={() => {
                setCurrentIndex(0);
      ...
      {isSlideshowOpen && photos.length > 0 && (
        <div className="fixed inset-0 bg-black z-50 flex flex-col items-center justify-between p-6">
          ...
          {/* Barre du bas avec contrôles manuels et indicateur */}
